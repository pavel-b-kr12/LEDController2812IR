...
void setPixelHeatColor (int Pixel, byte temperature) {
  ...
  // {
   ...
   //}else
   {
	...
	if(effSpeedH<4)
		leds[Pixel]=ColorFromPalette(fire_p, temperature, 255, LINEARBLEND); //? is need ", 255, LINEARBLEND"
	else
		leds[Pixel]=CHSV(effSpeedH+ constrain(map(temperature,0,211,0,60),0,60), temperature>200?0: map(temperature,0,200,200,0) ,temperature>100?255:map(temperature,0,100,0,255));
...
CRGB c, c1;
  ...
  leds[Pixel]=blend(c,c1,effSpeedH);
