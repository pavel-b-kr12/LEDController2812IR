...
void blendwave() {
  ...
  bgColor = blend(CHSV(beatsin8(4,0,255),255,255), CHSV(beatsin8(3,0,255),255,255), speed);

  uint8_t loc1 = beatsin8(10,0,NUM_LEDS-1);
  //uint8_t loc2 = effLength;
  
  if(effLength<20)
  fill_gradient_RGB(leds, 0, bgColor, loc1, gColor);
  else
  fill_gradient_RGB(leds, loc1, bgColor, lerp8by8(0,NUM_LEDS-1,effLength), gColor);
}