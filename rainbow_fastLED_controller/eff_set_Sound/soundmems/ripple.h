...
// Persistent local variables.
  ...
  switch (step) {
    ...
    case 0:
      leds[center] += ColorFromPalette(currentPalette, colour? colour+millis(), 255, currentBlending); // Display the first pixel of the ripple.
      step ++;
      break;
    ...
    default:                                                                    // Middle of the ripples.
      ...
      leds[(center + step + NUM_LEDS) % NUM_LEDS] += ColorFromPalette(currentPalette, colour? colour+millis(), 255/step*2, currentBlending);  // A spreading and fading pattern up the strand.
      leds[(center - step + NUM_LEDS) % NUM_LEDS] += ColorFromPalette(currentPalette, colour? colour+millis(), 255/step*2, currentBlending);  // A spreading and fading pattern down the strand.
      step ++;                                                                  // Next step.
      break;  
